...
export default async function handler(req, res) {
  ...
  const resultUndangan = await undangan.aggregate([
    {
      $group: {
        _id: { $month: "$createdAt" },
        data: { $push: "$$ROOT" },
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    const undanganCount =
      resultUndangan.find((item) => item._id === month)?.count || 0;
    ...
    if (undanganCount > 0) {
      dataUndangan.push(keteranganCount);
    } else {
      dataUndangan.push(null);
    }
  }
